...
export default function AIManagementTab() {
    ...
    const { data: checkHistory = [], isLoading: loadingHistory } = useQuery({
        ...
        queryFn: async () => {
            const history = await base44.entities.ModerationCheckHistory.list('-check_date', 10);
            return history;
        }
    });
    ...
    const savePromptMutation = useMutation({
        mutationFn: async ({ promptKey, promptText, description }) => {
            ...
            if (existing) {
                return base44.entities.AIPrompt.update(existing.id, { prompt_text: promptText });
            } else {
                return base44.entities.AIPrompt.create({
    ...
    const { data: moderationRounds = [], isLoading: loadingRounds, refetch: refetchRounds } = useQuery({
        ...
        queryFn: async () => {
            const allRounds = await base44.entities.ModerationRound.filter({ 
            ...
            const roundsWithLocations = await Promise.all(
                aiRounds.map(async (round) => {
                    const locations = await base44.entities.Location.filter({ id: round.location_id });
                    return {
                        ...round,
    ...
    const applyMutation = useMutation({
        mutationFn: async ({ roundId, locationId, fieldName, proposedValue, proposedTags, currentValue }) => {
            ...
            const response = await base44.functions.invoke('applyModerationRound', {
                roundId,
                locationId,
                fieldName,
                proposedValue,
    ...
    const rejectMutation = useMutation({
        mutationFn: async (roundId) => {
            setProcessingRoundId(roundId);
            const response = await base44.functions.invoke('rejectModerationRound', {
                roundId
            });
    ...
    const handleRunCheck = async () => {
        ...
        try {
            ...
            const response = await base44.functions.invoke('locationModerationAutomation', payload);
                                    ...
                                    <Button 
                                        onClick={async () => {
                                            try {
                                                ...
                                                const response = await base44.functions.invoke('reformatExistingOpeningHours');
