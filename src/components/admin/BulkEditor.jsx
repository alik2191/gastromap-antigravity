...
export default function BulkEditor({ isOpen, onOpenChange, rows = [], onSaved }) {
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      modifiedRows.forEach(m => {
        ...
        operations.push(base44.entities.Location.update(m.id, payload));
      });
      
      // Удаляем помеченные строки
      deletedRows.forEach(id => {
        operations.push(base44.entities.Location.delete(id));
      });
  ...
  const handleSendToModeration = async () => {
    ...
    setSaving(true);
    try {
      await Promise.all(Array.from(selected).map(id => base44.entities.Location.update(id, { status: 'pending' })));
